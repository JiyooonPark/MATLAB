...
[m1, n1] = size( a );
Z = zeros(m1, n1);
% xa has 98% of the information
% h : row v: vertical d:vertical 
xa = IDWT_2D( a, Z, Z, Z, lo, hi, size(x) );
xh = IDWT_2D( Z, h, Z, Z, lo, hi, size(x) );
xv = IDWT_2D( Z, Z, v, Z, lo, hi, size(x) );
xd = IDWT_2D( Z, Z, Z, d, lo, hi, size(x) );
